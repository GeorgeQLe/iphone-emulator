...
- [x] Step 8.8: Refactor native capability boundaries if needed while keeping validation green
  ...
  - Completion notes: completed as a no-op boundary review. The Swift runtime manifest value types, diagnostics native guidance mapping, TypeScript automation SDK manifest types and cloning, `README.md`, `examples/strict-mode-baseline/README.md`, and `docs/native-capabilities.md` remain aligned around deterministic fixture data. No premature native service behavior, host permission probing, live device dependency, live network access, or type-name drift was found.
  - Validation result: no source validation rerun was needed because Step 8.8 changed no runtime, diagnostics, SDK, renderer, or docs source files. Step 8.7 already proved the full validation surface green.

### Milestone: M7 Native Capability Registry and Manifest
**Acceptance Criteria:**
- [x] The spec and docs define what a supported native capability must include before implementation.
- [x] Runtime and automation contracts have typed manifest shapes, even if most capabilities are initially unsupported.
- [x] Unsupported native APIs produce structured diagnostics with suggested strict-mode mock alternatives.
- [x] No capability depends on live host permissions, live device state, or live network access by default.
