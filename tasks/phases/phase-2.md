...
- [x] Step 2.7: Refactor the UI tree and renderer boundary if needed while keeping the new tests green.
  ...
  - Completed on 2026-04-27 by aligning the browser renderer's `UITreeScene.kind` union with the Swift runtime contract and removing the unsupported `navigationStack` scene-kind variant from the renderer-only type surface.
