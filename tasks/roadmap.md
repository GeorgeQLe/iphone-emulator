...
**Parallelization:** implementation-safe
**Coordination Notes:** The Swift CLI and TypeScript wrapper can be built concurrently since the JSON output schema is the shared contract. The JSON schema should be defined early as the shared chokepoint. Integration tests tying both layers together run after both are implemented.
